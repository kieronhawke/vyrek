...
import { runLiveTick } from "@/lib/results/engine/sync/live";

export async function GET(request: Request) {
  try {
    const blocked = guard(request);
    if (blocked) return blocked;
    // Runs every minute; decides for itself which events are due, so the
    // schedule never has to know about race timings.
    ...
    const result = await runLiveTick(getSyncEngine(), { triggerSource: "cron" });
