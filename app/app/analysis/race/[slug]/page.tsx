...
export default async function RaceDetailPage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 md:py-10">
      ...
      <section className="mt-8 rounded-2xl border border-suth-border-subtle bg-suth-elevated/60 p-5">
        <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-suth-text-tertiary">
          Date
        </p>
        <p className="mt-1 text-base text-suth-text">{dateFmt}</p>
      </section>
      ...
      <section className="mt-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {race.categories.map((c) => (
            <span
              key={c}
              className="rounded-pill border border-suth-border bg-suth-elevated px-3 py-1 text-xs font-medium text-suth-text"
            >
              {c}
            </span>
          ))}
      ...
      <section className="mt-8">
        ...
        <ul role="list" className="mt-3 space-y-2">
          {PREP_CHECKLIST.map((item, i) => (
            <li
              key={i}
              className="flex items-start gap-3 rounded-lg border border-suth-border-subtle bg-suth-elevated/60 px-4 py-3"
            >
              <span
                aria-hidden
                className="mt-2 size-1.5 shrink-0 rounded-full bg-suth-accent"
              />
              <p className="text-sm text-suth-text">{item}</p>
            </li>
          ))}
        </ul>
      </section>
      ...
      {race.registrationOpen && race.registrationUrl ? (
        <div className="mt-10">
          ...
          <p className="mt-3 text-center font-mono text-[10px] uppercase tracking-[0.22em] text-suth-text-tertiary">
            Registration is handled by Hyrox, not Suth Performance.
          </p>
        </div>
      ) : (
        <div className="mt-10 rounded-pill border border-suth-border bg-suth-elevated px-5 py-3 text-center text-sm text-suth-text-secondary">
