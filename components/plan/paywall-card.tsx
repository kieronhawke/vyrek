...
export function PaywallCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-suth-border bg-suth-elevated p-7",
        variant === "card" && "text-center",
      )}
    >
      <h3 className="text-2xl font-black leading-tight tracking-[-0.02em] text-suth-text md:text-3xl">
        Unlock weeks 2-12
      </h3>
      <button
        type="button"
        ...
        disabled={loading}
        className="mt-6 inline-flex h-14 w-full items-center justify-center rounded-pill bg-suth-accent px-6 text-base font-semibold tracking-tight text-[#0A0A0A] transition-[background,transform,opacity] duration-fast ease-out hover:bg-suth-accent-hover active:scale-[0.98] disabled:opacity-60"
      >
