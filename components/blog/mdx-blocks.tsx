...
export function RaceAnalytics({
  ...
  return (
    <figure className="mt-10 overflow-hidden rounded-lg border border-suth-border-subtle bg-suth-elevated">
      <header className="flex flex-wrap items-baseline justify-between gap-3 border-b border-suth-border-subtle px-5 py-4 md:px-6">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-suth-accent">
            [ RACE ANALYTICS ]
          </p>
          <p className="mt-1 text-base font-bold text-suth-text md:text-lg">
            {athlete} · {event}
          </p>
        </div>
        <p className="font-mono text-lg font-bold tabular-nums text-suth-text md:text-xl">
          {totalTime}
        </p>
      </header>
      <table className="w-full text-sm md:text-base">
        <thead>
          <tr className="border-b border-suth-border-subtle text-left">
            <th className="px-5 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-suth-text-tertiary md:px-6">
              Station
            </th>
            <th className="px-5 py-2 text-right font-mono text-[10px] uppercase tracking-[0.18em] text-suth-text-tertiary md:px-6">
              Split
            </th>
            <th className="hidden px-5 py-2 text-right font-mono text-[10px] uppercase tracking-[0.18em] text-suth-text-tertiary md:table-cell md:px-6">
              Cumulative
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((s) => (
            ...
            >
              <td className="px-5 py-3 text-suth-text md:px-6">{s.station}</td>
              <td className="px-5 py-3 text-right font-mono tabular-nums text-suth-text md:px-6">
                {s.split}
              </td>
              <td className="hidden px-5 py-3 text-right font-mono tabular-nums text-suth-text-secondary md:table-cell md:px-6">
                {s.cumulative ?? ""}
              </td>
            </tr>
          ))}
...
export function Leaderboard({
  ...
  return (
    <figure className="mt-10 overflow-hidden rounded-lg border border-suth-border-subtle bg-suth-elevated">
      ...
      <ol role="list" className="divide-y divide-suth-border-subtle">
        {list.map((r) => (
          <li
            key={`${r.rank}-${r.name}`}
            className="grid grid-cols-[40px_1fr_auto] items-center gap-3 px-5 py-3 md:px-6"
          >
            <span className="font-mono text-sm font-bold tabular-nums text-suth-accent">
              {String(r.rank).padStart(2, "0")}
            </span>
            <span className="text-base font-medium text-suth-text">
              {r.name}
              {r.nationality ? (
                <span className="ml-2 font-mono text-[10px] uppercase tracking-[0.18em] text-suth-text-tertiary">
                  {r.nationality}
                </span>
              ) : null}
            </span>
            <span className="font-mono text-base font-bold tabular-nums text-suth-text">
              {r.time}
            </span>
          </li>
        ))}
      ...
      {caption ? (
        <figcaption className="border-t border-suth-border-subtle px-5 py-3 font-mono text-[10px] uppercase tracking-[0.22em] text-suth-text-tertiary md:px-6">
          {caption}
        </figcaption>
      ) : null}
    </figure>
...
export function TipCallout({
  ...
  return (
    <aside
      role="note"
      className={cn(
        "mt-10 grid gap-4 rounded-lg border border-suth-accent/30 bg-suth-accent/5 p-5 md:grid-cols-[120px_1fr] md:gap-6 md:p-6",
      )}
    >
      <div className="flex items-start gap-3 md:flex-col">
        <span
          aria-hidden
          className="flex size-12 items-center justify-center rounded-full border border-suth-accent/40 bg-suth-base/40 font-mono text-base font-bold text-suth-accent md:size-16 md:text-lg"
        >
          T
        </span>
        <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-suth-accent md:mt-2">
          [ COACH TIP ]
        </p>
      </div>
      <div>
        <p className="text-lg font-bold leading-tight text-suth-text md:text-xl">
          {title}
        </p>
        <div className="mt-3 text-base leading-relaxed text-suth-text [&>p:first-child]:mt-0 [&>p]:mt-3">
          {children}
        </div>
        {coach ? (
...
export function ComparisonTable({
  ...
  return (
    <figure className="mt-10 overflow-x-auto">
      <table className="w-full min-w-[480px] overflow-hidden rounded-lg border border-suth-border-subtle bg-suth-elevated text-sm md:text-base">
        <thead>
          <tr className="border-b border-suth-border-subtle bg-suth-base/40">
            <th className="px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[0.18em] text-suth-text-tertiary md:px-5" />
            {cols.map((c) => (
              <th
                key={c}
                className="px-4 py-3 text-left font-mono text-[11px] uppercase tracking-[0.22em] text-suth-text md:px-5"
              >
                {c}
              </th>
            ))}
        ...
        <tbody>
          {rs.map((r) => (
            ...
            >
              <th
                scope="row"
                className="px-4 py-3 text-left text-sm font-bold text-suth-text-secondary md:px-5"
              >
                {r.label}
              </th>
              {(Array.isArray(r.values) ? r.values : []).map((v, i) => (
                <td
                  key={`${r.label}-${i}`}
                  className="px-4 py-3 align-top text-suth-text md:px-5"
                >
                  {v}
                </td>
              ))}
      ...
      {caption ? (
        <figcaption className="mt-3 font-mono text-[10px] uppercase tracking-[0.22em] text-suth-text-tertiary">
          {caption}
        </figcaption>
      ) : null}
    </figure>
...
export function WorkoutDemoVideo({
  ...
  return (
    <figure className="mt-10 overflow-hidden rounded-lg border border-suth-border-subtle bg-suth-elevated">
      ...
      <div className="relative aspect-video bg-suth-base">
        {isFile ? (
          <video
            src={src}
            poster={poster}
          ...
          >
            ...
            <div
              aria-hidden
              className="absolute inset-0 bg-suth-base/40 transition-colors group-hover:bg-suth-base/20"
            />
            <span
              aria-hidden
              className="relative flex size-16 items-center justify-center rounded-full bg-suth-accent text-[#0A0A0A] transition-transform duration-base group-hover:scale-110 md:size-20"
            >
...
export function SledCalculator({
  ...
  return (
    <figure className="mt-10 rounded-lg border border-suth-border-subtle bg-suth-elevated p-5 md:p-6">
      <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-suth-accent">
        [ SLED CALCULATOR ]
      </p>
      <p className="mt-1 text-base font-bold text-suth-text md:text-lg">
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <label
          className="flex flex-col gap-2 text-sm text-suth-text-secondary"
          htmlFor={`${id}-bw`}
        >
          Bodyweight (kg)
          <input
            ...
            className="rounded-md border border-suth-border bg-suth-base px-3 py-2 text-base text-suth-text outline-none focus:border-suth-accent"
          />
        </label>
        <label
          className="flex flex-col gap-2 text-sm text-suth-text-secondary"
      ...
      <dl className="mt-6 grid grid-cols-2 gap-3">
        <div className="rounded-md border border-suth-border-subtle bg-suth-base/40 p-4">
          <dt className="font-mono text-[10px] uppercase tracking-[0.22em] text-suth-text-tertiary">
            Sled push
          </dt>
          <dd className="mt-2 text-2xl font-black tabular-nums text-suth-text md:text-3xl">
            {stats.push} kg
          </dd>
          <dd className="mt-1 font-mono text-xs text-suth-accent">
