 ...
 *
 * It answers seven questions, and it does not fix anything — an audit that
 ...
 *   8. Is anything stored that no ingestion run can account for?
 *
...
for (const r of results) {
  byDivision.set(r.division_id, (byDivision.get(r.division_id) ?? 0) + 1);
}
...
log(`  8. provenance          ${runCount} ingestion runs, ${quarantined} quarantined rows\n`);
