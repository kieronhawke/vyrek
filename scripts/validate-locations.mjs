...
for (const loc of registry) {
  ...
  results[loc.slug] = { publishable, populatedFields: populated, missingMandatory };
}
...
const lines = [
  ...
  "",
  `Gate: ≥${MIN_POPULATED_FIELDS} populated unique-data fields, including at least one`,
  ...
  `| Publishable (pass the gate) | ${publishable.length} |`,
  `| Enriched but blocked | ${enrichedBlocked.length} |`,
...
console.log(
  `Location gate: ${publishable.length}/${registry.length} publishable · ` +
    `${enrichedBlocked.length} enriched-but-blocked · ${bare} identity-only · ` +
    `report: docs/location-validator-report.md`,
);
