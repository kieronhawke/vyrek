/**
 * Seeds the gym layer for the international race cities from OpenStreetMap.
 *
...
const CITIES = path.join(ROOT, "data", "locations", "race-cities.json");
const ENRICH_DIR = path.join(ROOT, "data", "locations", "enrichment-intl");
...
const cities = JSON.parse(readFileSync(CITIES, "utf8")).cities.filter(
  (c) => (!only || only.has(c.slug)) && c.lat != null,
);
